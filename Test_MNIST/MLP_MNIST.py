...
class MLP_SR(nn.Module):
    ...
    def forward(self, x):
        x = x.view(-1, 8 * 8)  # Flatten ảnh 8x8 thành vector
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        x = F.relu(self.fc3(x))
        x = torch.sigmoid(self.fc4(x))  # Output trong khoảng [0,1]
        x = x.view(-1, 1, 64, 64)  # Reshape về ảnh 64x64
        return x

# Dataset
class SuperResolutionDataset(torch.utils.data.Dataset):
    def __init__(self, dataset, selected_digits):
        self.dataset = [(data[0], data[1]) for data in dataset if data[1] in selected_digits]
...
def test_metrics_mlp_sr(model, test_loader, output_metrics_path="./Output_MNIST/output_mlp/test_metrics.txt"):
    model.eval()
    total_psnr, total_ssim = 0, 0
    with torch.no_grad():
...
def train_mlp_sr(model, train_loader, val_loader, epochs=25, lr=0.005, patience=5):
    ...
    plt.legend()
    plt.grid()
    plt.savefig("./Output_MNIST/output_mlp/psnr_mlp_plot.pdf")
    plt.close()

    return model, total_train_time

def test_mlp_sr(model, test_loader, output_path="./Output_MNIST/output_mlp/results_mlp.pdf"):
...
def main():
    train_dataset_file = "./train_data_set/MLP/MNIST/train_dataset.pt"
    val_dataset_file = "./val_data_set/MLP/MNIST/val_dataset.pt"
    test_dataset_file = "./test_data_set/MLP/MNIST/test_dataset.pt"
    output_metrics_path = "./Output_MNIST/output_mlp/test_metrics.txt"
