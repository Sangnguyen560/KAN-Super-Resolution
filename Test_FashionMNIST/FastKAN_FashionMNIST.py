...
def test_metrics_fastkan_sr(model, test_loader, output_metrics_path="./Output_FashionMNIST/output_fastkan/test_metrics.txt"):
    model.eval()
    total_psnr, total_ssim = 0, 0
    with torch.no_grad():
...
def train_fastkan_sr(model, train_loader, val_loader, epochs=25, lr=0.005, patience=5):
    ...
    plt.legend()
    plt.grid()
    plt.savefig("./Output_FashionMNIST/output_fastkan/psnr_plot.pdf")
    plt.close()

    return model, total_train_time

def test_fastkan_sr(model, test_loader, output_path="./Output_FashionMNIST/output_fastkan/results_fastkan.pdf"):
...
def main():
    train_dataset_file = "./train_data_set/FastKAN/FashionMNIST/train_dataset.pt"
    val_dataset_file = "./val_data_set/FastKAN/FashionMNIST/val_dataset.pt"
    test_dataset_file = "./test_data_set/FastKAN/FashionMNIST/test_dataset.pt"
    output_metrics_path = "./Output_FashionMNIST/output_fastkan/test_metrics.txt"
    ...
    if total_samples < 5000:
        ...
        val_size = total_samples - train_size  # 20% val
    else:
    ...
    model = FastKAN_SR(num_grids=10, hidden_dim=256)
