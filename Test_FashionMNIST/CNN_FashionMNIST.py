...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        return F.leaky_relu(x + residual, negative_slope=0.2)
...
class CNN_SR(nn.Module):
    def __init__(self):
        ...
        self.upsample1 = nn.ConvTranspose2d(64, 64, kernel_size=4, stride=2, padding=1, bias=False)
        self.upsample2 = nn.ConvTranspose2d(64, 64, kernel_size=4, stride=2, padding=1, bias=False)
        self.upsample3 = nn.ConvTranspose2d(64, 64, kernel_size=4, stride=2, padding=1, bias=False)
        self.conv_final = nn.Conv2d(64, 1, kernel_size=3, padding=1, bias=False)

    def forward(self, x):
        x = F.leaky_relu(self.bn1(self.conv1(x)), negative_slope=0.2)
        x = self.res_block1(x)
        x = self.res_block2(x)
        x = F.leaky_relu(self.upsample1(x), negative_slope=0.2)
        x = F.leaky_relu(self.upsample2(x), negative_slope=0.2)
        x = F.leaky_relu(self.upsample3(x), negative_slope=0.2)
...
def validate_cnn_sr(model, val_loader):
    model.eval()
...
def test_metrics_cnn_sr(model, test_loader, output_metrics_path="./Output_FashionMNIST/output_cnn/test_metrics.txt"):
    model.eval()
    total_psnr, total_ssim = 0, 0
    with torch.no_grad():
...
def train_cnn_sr(model, train_loader, val_loader, epochs=25, lr=0.005, patience=5):
    model.to(device)
    ...
    for epoch in range(epochs):
        ...
        val_psnr, val_ssim = validate_cnn_sr(model, val_loader)
    ...
    plt.legend()
    plt.grid()
    plt.savefig("./Output_FashionMNIST/output_cnn/psnr_plot.pdf")
    plt.close()

    return model, total_train_time

def test_cnn_sr(model, test_loader, output_path="./Output_FashionMNIST/output_cnn/results_cnn.pdf"):
...
def main():
    train_dataset_file = "./train_data_set/CNN/FashionMNIST/train_dataset.pt"
    val_dataset_file = "./val_data_set/CNN/FashionMNIST/val_dataset.pt"
    test_dataset_file = "./test_data_set/CNN/FashionMNIST/test_dataset.pt"
    output_metrics_path = "./Output_FashionMNIST/output_cnn/test_metrics.txt"
    ...
    # Chia dữ liệu: 4000 train, 1000 val
    ...
    cnn_model = CNN_SR()
    total_params = count_params(cnn_model, header="Model Parameters Before Training")

    trained_model, train_time = train_cnn_sr(cnn_model, train_loader, val_loader, epochs=25)
    ...
    test_time = test_cnn_sr(trained_model, test_loader)
    test_psnr, test_ssim = test_metrics_cnn_sr(trained_model, test_loader, output_metrics_path)
